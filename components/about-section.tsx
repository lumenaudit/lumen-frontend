      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid md:grid-cols-[1.3fr_1fr] gap-16 lg:gap-20 items-center">
          ...
          <div className="ps-0 md:ps-8">
            ...
            <p className="text-lg text-primary/70 mb-6 leading-relaxed">{t("paragraph1")}</p>
            <p className="text-lg text-primary/70 mb-8 leading-relaxed">{t("paragraph2")}</p>

            <div className="space-y-4">
              {(["point1", "point2", "point3"] as const).map((key) => (
                <div key={key} className="flex gap-4">
                  <div className="h-2 w-2 bg-accent flex-shrink-0 mt-1.5"></div>
                  <p className="text-primary font-semibold">{t(key)}</p>
                </div>
              ))}
            </div>
          </div>
